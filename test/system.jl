...
function simulate_kalman_filter_over_time(; x, P, F, Q, R, H, num_iterations = 1000)
    ...
    true_states_over_time,
    est_states_over_time,
    est_state_vars_over_time,
    nis_over_time,
    innovation_over_time,
    innovation_vars_over_time
end
...
function test_innovation(niss, innovations, innovation_vars)
    ...
    if eltype(innovations) <: Complex
        @test all(
            calc_sigma_bound_test(real.(innovations), innovation_vars / 2; atol = 0.02),
        )
        @test all(
            calc_sigma_bound_test(imag.(innovations), innovation_vars / 2; atol = 0.02),
        )
    else
...
function test_state_errors(state_errors, est_state_vars)
    ...
    if eltype(state_errors) <: Complex
        @test all(
            calc_sigma_bound_test(real.(state_errors), est_state_vars / 2; atol = 0.08),
        )
        @test all(
            calc_sigma_bound_test(imag.(state_errors), est_state_vars / 2; atol = 0.08),
        )
    else
