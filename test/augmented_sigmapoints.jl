@testset "Augmented Sigma points" begin
    ...
    @testset "Covariance" begin
        ...
        𝓨 = KalmanFilters.TransformedSigmaPoints(
            ones(5),
            hcat(ones(5, 5) .* 4, ones(5, 5) .* 2, ones(5, 5) .* 3, ones(5, 5) .* 1),
            weight_params,
        )
        χ = KalmanFilters.AugmentedSigmaPoints(
            x,
            P_chol,
            LowerTriangular(collect(noise)),
            weight_params,
        )
        P = @inferred KalmanFilters.cov(χ, 𝓨)
        weight_0, weight_i = KalmanFilters.calc_cov_weights(𝓨)
        @test P ==
              weight_i .* LowerTriangular(ones(5, 5)) * ones(5, 5)' .* 4 .-
    ...
    @testset "Transform sigma points" begin
        ...
        𝓨_temp =
            KalmanFilters.TransformedSigmaPoints(zeros(2), zeros(2, 8), other_weight_params)
        F!(y, x) = y .= x .* 2
        F!(y, x, n) = y .= x .* 2 .+ n
