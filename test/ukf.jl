@testset "Unscented Kalman filter" begin
    ...
    @testset "Create pseudo Sigma points" begin
        weighted_P_chol = Cholesky(ones(5,5), 'L', 0)
        χ_diff_x_pseudo = KalmanFilter.create_pseudo_sigmapoints(weighted_P_chol)
        @test χ_diff_x_pseudo.xi_P_plus == weighted_P_chol.L
        @test χ_diff_x_pseudo.xi_P_minus == -weighted_P_chol.L
