module KalmanFilter
    ...
    function time_update(𝐱_init, 𝐏_init, 𝐱, 𝐏, scales, 𝐟_or_𝐓, 𝐐, used_states, reset_unused_states)
        part_𝐱, part_𝐏 = filter_states(𝐱, 𝐏, used_states)
        time_update_output = _time_update(part_𝐱, part_𝐏, scales, 𝐟_or_𝐓, 𝐐)
        (𝐲, 𝐇, 𝐑) -> measurement_update(𝐱_init, 𝐏_init, 𝐱, 𝐏, time_update_output..., scales, 𝐲, 𝐇, 𝐑, used_states, reset_unused_states)
    end
    
    function time_update(𝐱_init, 𝐏_init, 𝐱, 𝐏, scales, 𝐟_or_𝐓, 𝐐::Augment, used_states, reset_unused_states)
        part_𝐱, part_𝐏 = filter_states(𝐱, 𝐏, used_states)
        part_𝐱ᵃ, part_𝐏ᵃ = augment(part_𝐱, part_𝐏, 𝐐)
        time_update_output = _time_update(part_𝐱ᵃ, part_𝐏ᵃ, scales, 𝐟_or_𝐓, 0)
        (𝐲, 𝐇, 𝐑) -> measurement_update(𝐱_init, 𝐏_init, 𝐱, 𝐏, time_update_output..., scales, 𝐲, 𝐇, 𝐑, used_states, reset_unused_states)
    end
    
    function time_update(𝐱_init, 𝐏_init, 𝐱, 𝐏, scales, 𝐟_or_𝐓, 𝐐::Augment, 𝐑::Augment, used_states, reset_unused_states)
        part_𝐱, part_𝐏 = filter_states(𝐱, 𝐏, used_states)
        part_𝐱ᵃ, part_𝐏ᵃ = augment(part_𝐱, part_𝐏, 𝐐, 𝐑)
        time_update_output = _time_update(part_𝐱ᵃ, part_𝐏ᵃ, scales, 𝐟_or_𝐓, 0)
        (𝐲, 𝐇) -> measurement_update(𝐱_init, 𝐏_init, 𝐱, 𝐏, time_update_output..., scales, 𝐲, 𝐇, used_states, reset_unused_states)
    end
